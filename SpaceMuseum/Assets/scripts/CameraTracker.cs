...
public class CameraTracker : MonoBehaviour
{
    ...
    [Header("鼠标控制关联")]
    public ModelMouseControl modelMouseControl; // 鼠标控制脚本引用
    // 组件映射字典
    ...
    void LerpToTarget()
    {
        ...
        if (t >= 1f)
        {
            isLerping = false;
            lerpTime = 0f;
            // 相机移动完成后，若聚焦卫星，设置鼠标控制
            if (!isMainView && targetSatellite != null && modelMouseControl != null)
            {
                modelMouseControl.SetupForModel(targetSatellite);
            }
        }
    }
    // 聚焦到指定卫星（核心功能）
    ...
    public void ReturnToMainView()
    {
        ...
        isMainView = true;
        // 禁用鼠标控制
        if (modelMouseControl != null)
        {
            modelMouseControl.DisableControl();
        }
        // 1. 恢复所有卫星的运动
